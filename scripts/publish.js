...
console.log('🔍 Vérification de l\'authentification npm...');
try {
...
console.log('🔍 Vérification de la version de npm...');
try {
  const npmVersion = execSync('npm --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
  const [major] = npmVersion.split('.').map(Number);
  if (major < 7) {
    console.warn(`⚠️  Version de npm ${npmVersion} détectée. Il est recommandé d'utiliser npm 7.x ou supérieur.`);
  } else {
    console.log(`✅ npm version ${npmVersion} compatible`);
  }
} catch (error) {
  console.warn('⚠️  Impossible de vérifier la version de npm');
}
