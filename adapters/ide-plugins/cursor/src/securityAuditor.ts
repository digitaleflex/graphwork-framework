...
`;
        ...
        const auditDocument = await cursor.workspace.openTextDocument({
          content: response.content,
          language: 'markdown'
        });

        await cursor.window.showTextDocument(auditDocument, {
          preview: false,
          viewColumn: cursor.ViewColumn.Beside
        });

        cursor.window.showInformationMessage('Security audit completed successfully!');
      });
    } catch (error) {
      console.error('Error auditing security:', error);
      cursor.window.showErrorMessage('Failed to audit security. Please check your API key and network connection.');
    }
  }
}