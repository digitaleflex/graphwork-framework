...
`;
        ...
        const docDocument = await cursor.workspace.openTextDocument({
          content: response.content,
          language: 'markdown'
        });

        await cursor.window.showTextDocument(docDocument, {
          preview: false,
          viewColumn: cursor.ViewColumn.Beside
        });

        cursor.window.showInformationMessage('Documentation generated successfully!');
      });
    } catch (error) {
      console.error('Error generating documentation:', error);
      cursor.window.showErrorMessage('Failed to generate documentation. Please check your API key and network connection.');
    }
  }
}