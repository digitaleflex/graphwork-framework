  ...
  constructor(apiKey: string, model: string, temperature: number, maxTokens: number) {
    ...
    this.axiosInstance = axios.create({
      ...
      headers: {
        ...
        'User-Agent': 'GraphWork-VSCode-Extension/2.0.0'
      }
    });
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          vscode.window.showErrorMessage('Invalid GraphWork API key. Please check your settings.');
        } else if (error.response?.status === 429) {
          vscode.window.showErrorMessage('Rate limit exceeded. Please try again later.');
        } else if (error.code === 'ECONNABORTED') {
          vscode.window.showErrorMessage('Request timeout. Please check your network connection.');
        } else {
          vscode.window.showErrorMessage(`GraphWork API error: ${error.message}`);
        }
